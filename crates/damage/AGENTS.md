...
the Python surface (`nucleide.damage`, `damage_*` in `_internal`), and the
...
- SPECTER (ANL/FPP/TM-197, US-gov PD) is the validation oracle ONLY:
  `validation/damage_vs_specter.py` transcribes a handful of output spots
  (Tables V/VI, HFIR-CTR32) and folds them at report print precision
  (5 digits, tol 1e-4 relative). No SPECTER table is vendored; a future
  vendoring step is a separate documented decision.
...
- Out of scope (do not expand here): PKA-spectra solving, displacement
  table vendoring, transport solving, IAEA/IRDFF/TENDL consumption
...
- `cargo test -p nucleide-damage` (closed-form limits, piecewise gates,
  fold conventions, UQ k-SE gates).
